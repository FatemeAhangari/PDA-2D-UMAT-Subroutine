     ...
     3 PROPS(NPROPS),COORDS(3),DROT(3,3),DFGRD0(3,3),DFGRD1(3,3)
      ...
      INTEGER i,j
      DOUBLE PRECISION Dgrd(6),DMG(8),FI(4),STRANT(NTENS)
!C Data
!C=========================================================================		  
...
!C Calculation
     ...
     1 NU23,NU21,NU31,NU32,NTENS,DEG,DMG)
      ...
      ! Failure Index
      FI(1:4) = 0.0D0
      CALL FailureIndex(STRESS,Xt,Xc,Yt,Yc,Zt,Zc,S12,S13,S23,FI
     1 ,NTENS)
      ! Check Failure Index
...
!C=========================================================================	
     ...
     1 NU13,NU23,NU21,NU31,NU32,DEG,DMG(8)
      INTEGER NTENS
      IF(DEG.EQ.1) THEN
     	E11  = E11  * abs(DMG(7))
     	E22  = E22  * abs(DMG(7) * DMG(8)) 
     	E33  = E33  * abs(DMG(7) * DMG(8))
     	NU12 = NU12 * abs(DMG(7) * DMG(8))
     	NU13 = NU13 * abs(DMG(7))          
     	NU23 = NU23 * abs(DMG(7))          
     	G12  = G12  * abs(DMG(7)) 
     	G13  = G13  * abs(DMG(7))          
     	G23  = G23  * abs(DMG(7))          
     	NU21 = NU21 * abs(DMG(7) * DMG(8)) 
     	NU31 = NU31 * abs(DMG(7))          
     	NU32 = NU32 * abs(DMG(7))          
      ELSE
...
!C=========================================================================	
      SUBROUTINE FailureIndex(STRESS,Xt,Xc,Yt,Yc,Zt,Zc,S12,S13,S23,FI
     1 ,NTENS)
      INCLUDE 'ABA_PARAM.INC'
      INTEGER NTENS
      DOUBLE PRECISION STRESS(NTENS),Xt,Xc,Yt,Yc,Zt,Zc,S12,S13,S23,FI(4)
	  
      IF (STRESS(1).GT.0) THEN
	  ! Fiber Tension
      FI(1) = (STRESS(1)/Xt)**2
     1 +(1/(S12**2))*(STRESS(4)**2+STRESS(5)**2)
      ELSE
      ! Fiber Compression
      FI(2) = (-1)*(STRESS(1)/Xc)
      END IF
      ...
      ! Matrix Tension
       FI(3) = ((STRESS(2)+STRESS(3))**2)/(Yt**2)
     1 + (STRESS(4)**2+STRESS(5)**2
     2 +STRESS(6)**2-STRESS(2)*STRESS(3))/(S12**2)
      ELSE
      ! Matrix Compression
      FI(4) = (1/Yc)*((Yc/(2*S23))**2-1)*(STRESS(2)+STRESS(3))
     1 + ((STRESS(2)+STRESS(3))**2)/(4*S23**2)
     2 + (1/(S23**2))*(STRESS(6)**2-STRESS(2)*STRESS(3))
     3 + (1/(S12**2))*(STRESS(4)**2-STRESS(5)**2)
      END IF
